                ...
                match stored_version {
                    Some(bytes) => {
                        ...
                        if stored != config.format_version {
                            return Err(SochError::FormatVersion {
                                got: stored,
            ...
            for dim in active_dims {
                ...
                let mut bitmap = match existing {
                    Some(bytes) => RoaringBitmap::deserialize_from(bytes.as_slice())
                        .map_err(|e| SochError::Internal(format!("roaring decode: {e}")))?,
                ...
                bitmap
                    ...
                    .map_err(|e| SochError::Internal(format!("roaring encode: {e}")))?;
    ...
    pub fn supersede(&mut self, older: EpisodeId, newer: EpisodeId) -> Result<()> {
        let tx = self.db.begin_write()?;
        {
            ...
            let newer_bytes = episodes
                .get(newer.raw())?
                .ok_or(SochError::UnknownEpisode(newer.raw()))?
                .value();
            let newer_ep: Episode = decode(&newer_bytes)?;

            let older_bytes = episodes
                .get(older.raw())?
                .ok_or(SochError::UnknownEpisode(older.raw()))?
                .value();
    ...
    pub fn export_jsonl(&self, mut writer: impl Write) -> Result<()> {
        ...
        for entry in table.iter()? {
            ...
            let line = serde_json::to_string(&record)
                .map_err(|e| SochError::Internal(format!("json encode: {e}")))?;
    ...
    pub fn import_jsonl(&mut self, reader: impl std::io::Read) -> Result<u32> {
        ...
        for line in reader.lines() {
            ...
            let record: ExportRecord = serde_json::from_str(&line)
                .map_err(|e| SochError::Internal(format!("json decode: {e}")))?;
            if record.hv.len() != D_BYTES {
                return Err(SochError::Internal(format!(
...
fn encode<T: serde::Serialize>(value: &T) -> Result<Vec<u8>> {
    bincode::serialize(value).map_err(|e| SochError::Internal(format!("bincode encode: {e}")))
}

fn decode<T: for<'de> serde::Deserialize<'de>>(bytes: &[u8]) -> Result<T> {
    bincode::deserialize(bytes).map_err(|e| SochError::Internal(format!("bincode decode: {e}")))
}
