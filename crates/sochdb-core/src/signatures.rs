...
impl SignatureFile {
    ...
    pub fn open(path: impl Into<PathBuf>) -> Result<Self> {
        ...
        if file_len == 0 {
            ...
            Err(SochError::CorruptSignature { offset: 0, path })
        } else {
            // Existing file. Verify header.
            let mmap = unsafe { MmapMut::map_mut(&file)? };
            if mmap[0..8] != MAGIC {
                return Err(SochError::CorruptSignature { offset: 0, path });
            }
    ...
    pub fn read(&self, offset: u64) -> Result<HV> {
        ...
        if offset < HEADER_BYTES || offset + HV_SLOT > next {
            return Err(SochError::SignatureOutOfBounds { offset, len: next });
        }
