    ...
    fn load_episodes_with_signatures(&self) -> Result<Vec<(Episode, HV)>> {
        ...
        for entry in table.iter()? {
            ...
            let ep: Episode = bincode::deserialize(&bytes)
                .map_err(|e| SochError::Internal(format!("decode ep: {e}")))?;
        ...
        for cluster in clusters {
            ...
            let tx = self.db.begin_write()?;
            {
                ...
                let bytes = bincode::serialize(&atom)
                    .map_err(|e| SochError::Internal(format!("encode atom: {e}")))?;
    ...
    fn write_log_entry(&self, entry: &ConsolidationLogEntry) -> Result<()> {
        let tx = self.db.begin_write()?;
        {
            ...
            let bytes = bincode::serialize(entry)
                .map_err(|e| SochError::Internal(format!("encode log: {e}")))?;
        ...
        let id_value: u64;
        {
            ...
            id_value = match raw {
                Some(bytes) => bincode::deserialize(&bytes)
                    .map_err(|e| SochError::Internal(format!("decode atom-id: {e}")))?,
                None => 1,
            };
            let next = bincode::serialize(&(id_value + 1))
                .map_err(|e| SochError::Internal(format!("encode atom-id: {e}")))?;
